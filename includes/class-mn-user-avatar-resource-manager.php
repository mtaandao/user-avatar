...
class MN_User_Avatar_Resource_Manager {
  static $instance = NULL;

  public $marker = '<!-- mn_user_avatar_resource_manager_marker -->';
   ...
   * 2) Ensure that mn_print_footer_scripts() is called
   * @since 1.9.5
   */
  function __construct() {
    ...
    add_action('mn_footer', array(&$this, 'print_marker'), -1);
  }

  /**
   * Created early as possible in the mn_footer action this is the string to which we
  ...
  function validate_request() {
    $retval = TRUE;
    if(is_admin()) {
      if(isset($_REQUEST['page']) && !preg_match("#^(mn_user_avatar)#", $_REQUEST['page'])) {
        $retval = FALSE;
      }
    }
    if(strpos($_SERVER['REQUEST_URI'], 'admin/update') !== FALSE) {
      $retval = FALSE;
    } elseif(isset($_GET['display_gallery_iframe'])) {
      $retval = FALSE;
    } elseif(defined('ADMIN') && ADMIN && defined('DOING_AJAX') && DOING_AJAX) {
      $retval = FALSE;
  ...
  function start_buffer() {
    if(defined('MNUA_DISABLE_RESOURCE_MANAGER') && MNUA_DISABLE_RESOURCE_MANAGER) {
      return;
    }
    if(apply_filters('run_mnua_resource_manager', $this->valid_request)) {
      ob_start(array(&$this, 'output_buffer_handler'));
      ob_start(array(&$this, 'get_buffer'));
      add_action('mn_print_footer_scripts', array(&$this, 'get_resources'), 1);
   ...
   * in the shutdown function after running mn_print_footer_scripts()
   * @since 1.9.5
   * @param string $content
   * @return mixed
   */
  ...
  function output_buffer($in_shutdown=FALSE) {
    ...
    if(!$this->wrote_footer) {
      ...
      if(defined('W3TC') && defined('MN_DEBUG') && MN_DEBUG && !is_admin()) {
        if(!defined('DONOTCACHEPAGE')) define('DONOTCACHEPAGE', TRUE);
        if(!did_action('mn_footer')) {
          error_log("We're sorry, but your theme's page template didn't make a call to mn_footer(), which is required by User Avatar. Please add this call to your page templates.");
        } else {
          error_log("We're sorry, but your theme's page template didn't make a call to mn_print_footer_scripts(), which is required by User Avatar. Please add this call to your page templates.");
        }
